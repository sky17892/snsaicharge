...
class FortuneScriptItem(BaseModel):
    video_id: int = Field(description="영상 번호 (1~20, 고유 식별자)")
    ...
    # --- 페르소나·인물 일관성 (전체 20편에서 동일/제한된 인물군 유지) ---
...
class FortuneScriptBatchResponse(BaseModel):
    service_name: str
    total_count: int = Field(description="총 영상 편수 (기본 20편: 사용법 안내 5편 + 에피소드·스토리형 15편)")
    how_to_use_count: int = Field(default=5, description="사용법 안내 영상 편수")
    episode_count: int = Field(default=15, description="에피소드·스토리형 영상 편수")
